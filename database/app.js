...
// Find the buildTable function in your original app.js and update this part:
function buildTable(results) {
  if(!results.length) return '';
  const rows = results.map(r => {
    // We map your DB's 'url' (which was sent as 'arxiv_id') back to a link
    const openUrl = r.arxiv_id || (r.doi ? `https://doi.org/${r.doi}` : '#');
    ...
    return `<tr>
      ...
      <td class="td-cat">${esc(r.categories || 'General')}</td>
      <td class="td-links">
        <button class="link-btn" onclick="window.open('${openUrl}','_blank')">🔗 Open</button>
      </td>
    </tr>`;
  }).join('');
  return `<div class="table-wrap"><table class="results-table">
      <thead><tr><th>Title</th><th>Authors</th><th>Year</th><th>Keywords</th><th>Links</th></tr></thead>
